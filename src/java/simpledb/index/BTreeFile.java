...
public class BTreeFile implements DbFile {
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		BTreeEntry entry = new BTreeEntry(midField, page.getId(), rightPage.getId());
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int curTuplesSum = page.getNumTuples();
		int siblingTuplesSum = sibling.getNumTuples();
		int targetTuplesSum = (curTuplesSum + siblingTuplesSum) / 2;
		while(curTuplesSum < targetTuplesSum) {
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int curEntrySum = page.getNumEntries();
		int siblingEntrySum = leftSibling.getNumEntries();
		int targetEntrySum = (curEntrySum + siblingEntrySum) / 2;
		...
		while(curEntrySum < targetEntrySum) {
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int curEntrySum = page.getNumEntries();
		int siblingEntrySum = rightSibling.getNumEntries();
		int targetEntrySum = (curEntrySum + siblingEntrySum) / 2;
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		if(rrPageId == null) {
			...
			BTreeLeafPage rrPage = (BTreeLeafPage) getPage(tid, dirtypages, rrPageId, Permissions.READ_WRITE);
			rrPage.setLeftSiblingId(leftPage.getId());
			dirtypages.put(rrPageId, rrPage);
		}
